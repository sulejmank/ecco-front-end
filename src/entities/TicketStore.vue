  ...
  <div>
      ...
      <v-layout
        ...
        >
          ...
          <v-flex sm5 md3 hidden-xs-only>
              <h3 style="text-decoration:underline">Avio Karta</h3>
              <ul style="list-style-type: none; padding:0">
          ...
          <ticket-details :ticket="ticketSummary"></ticket-details>
        </v-layout>
    </v-card>
    <v-stepper v-model="step" vertical>
      ...
      <v-stepper-content step="1">
        ...
        <v-divider></v-divider>
        <v-subheader>Avio Karta</v-subheader>
        <ticket :ticket="ticket" :passangers="passangers" ref="ticket" @removePassanger="removePassanger"></ticket>
        <v-layout row wrap>
...
export default {
  ...
  data () {
    return {
      ...
      ticket: new Ticket(this.$route.params.type),
  ...
  computed: {
    ...
    getHeader () {
      return this.$route.params.type == 'bus' ? 'Nova Bus Karta' : 'Nova Avio Karta' 
    },
    ticketSummary () {
      let ticket = this.ticket
      ticket.Client = this.customer
      ticket.numberOfPassangers = 1 + this.passangers.length
      return ticket
    }
  },
  components: {
  ...
  methods: {
    ...
    async getPlan (id) {
      ...
      this.loadingMessage = 'Učitavanje avionske karte u toku'
      try {
    ...
    async checkoutEnd () {
      if (this.$refs.checkout.valid) {
        ...
        this.showLoading = true
        try {
          this.loadingMessage = 'Kreiranje avio karte u toku'
          // Save Customer
          ...
          this.proceed = true
          this.loadingMessage = 'Kreiranje avio karte u uspešno'
          this.showLoading = false
          router.push({ path: `/dashboard/plans` })
        } catch (error) {
          this.loadingMessage = 'Kreiranje avio karte u neuspešno'
          this.showLoading = false
          console.error(error)
    ...
    async checkoutUpdate () {
      if (this.$refs.checkout.valid) {
        ...
        // TODO: Implement Error Handling
        try {
          this.loadingMessage = 'Izmena avio karte u toku'
          this.showLoading = true
          // Save Customer
          let client = await this.api.saveCustomer(this.customer)

          // Save Destinations
          // if (this.ticket.putovanjeOdObj.id !== undefined)
          let destinationId = await this.api.saveDestination(this.ticket.putovanjeOdObj)
          let arrivalId = await this.api.saveDestination(this.ticket.putovanjeDoObj)
          ...
          this.loadingMessage = 'Kreiranje avio karte je uspela'
          this.showLoading = false
          // TODO: Create File from Plan
          this.proceed = true
          router.push({ path: `/dashboard/plans` })
        } catch (error) {
          this.loadingMessage = 'Izmena avio karte nije uspela'
          this.showLoading = false
          console.error(error)
          toast.error('Neuspesno upisivanje prodaje ' + error.toString())
        }
